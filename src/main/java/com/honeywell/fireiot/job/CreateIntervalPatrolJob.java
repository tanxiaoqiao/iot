 ...
 * @description: 间隔周期生成patrol 需要排除非工作日
 * @date:1/28/2019
 */
@Component
@DisallowConcurrentExecution
public class CreateIntervalPatrolJob implements Job {


    private static final Logger LOGGER = LoggerFactory.getLogger(CreateIntervalPatrolJob.class);
...
//        if (poll.getDayOfWeeks() != null && poll.getDayOfWeeks().length > 0) {
//            for (int i = 0; i < poll.getDayOfWeeks().length; i++) {
//                if (week == poll.getDayOfWeeks()[i]) {
//                    LOGGER.info("休息日：{}", poll.getDayOfWeeks()[i]);
//                    shouldWork = false;
//                    return;
//                }
//            }
...
//        }
        ...
        if (poll.getUseHoliday()) {
            ...
            if (quartzCalendar != null) {
                while (quartzCalendar.isDayExcluded(instance) || day == Calendar.SATURDAY || day == Calendar.SUNDAY) {
                    instance.add(Calendar.DAY_OF_YEAR, 1);
                }
            }
        }
        String current = DateFormatUtils.format(instance.getTime(), "yyyy-MM-dd");
